...
% Testing dependence on the beta parameter
% generates Figure 3.2 and Table 3.1 in the paper
...
for j = 1:5 
    ...
    bet = 10^(-j); % penalisation parameter
    nx = 12;       % interior grid pts in space
    x = chebpts(nx+2); x = x(2:end-1);
    [x1,x2] = meshgrid(x); x1 = x1(:); x2 = x2(:);
    L = chebop(-1,1);
    L.op = @(x,u) diff(u,2);
    L = L(nx+2); L = L(2:end-1,2:end-1);
    I = eye(nx);
    ...
    outer = max(find(out.nrmerru <= 1e-10,1,'first'),find(out.nrmerrv <= 1e-10,1,'first'));
    ...
    disp([ '$' , sprintf('%0.0e', bet) , '$ & ' , sprintf('%d',outer-1) , ' & ' , inner , ' & ' , sprintf('%d',total) , ' & $' , sprintf('%0.3g', final) , '$ & ' , sprintf('%0.3g', min(times)) , ' \\' ])
end

%%
figure(102)
semilogy([0,10],[1e-10,1e-10],'k--')
legend('\beta = 1e-1','\beta = 1e-2','\beta = 1e-3','\beta = 1e-4','\beta = 1e-5')
title('Schloegl spectral: error in computed state')
xlabel('Newton iteration')
mypdf('example_Schloegl_spectral_beta_1',.75,1.3)
figure(103)
semilogy([0,10],[1e-10,1e-10],'k--')
legend('\beta = 1e-1','\beta = 1e-2','\beta = 1e-3','\beta = 1e-4','\beta = 1e-5')
title('Schloegl spectral: error in computed adjoint')
xlabel('Newton iteration')
mypdf('example_Schloegl_spectral_beta_2',.75,1.3)
